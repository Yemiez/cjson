...
int cjson_partial_parse(cjson_context* ctx, cjson_value** out)
{
    ...
    if (c == '{') {
        (*out)->type = cjson_object;
        cjson_consume(ctx);
    }
    else if (c == '[') {
        (*out)->type = cjson_array;
        cjson_consume(ctx);
    }
    else if (c == '"') {
        ...
        if (punctflag) {
            (*out)->flags |= cjson_flag_double;
            (*out)->doubleval = strtod(buf, NULL);
        }
        else {
            (*out)->flags |= cjson_flag_integer;
...
int cjson_is_integer(cjson_value* v) { return v->type == cjson_number && v->flags & cjson_flag_integer; }
int cjson_is_object(cjson_value* v) { return v->type == cjson_object; }
int cjson_is_array(cjson_value* v) { return v->type == cjson_array; }
...
int cjson_is_boolean(cjson_value* v) { return v->type == cjson_boolean; }
int cjson_is_null(cjson_value* v) { return v->type == cjson_null; }
...
void cjson_set_double(cjson_value* v, double d)
{
    ...
    v->flags &= ~cjson_flag_integer;
    v->flags |= cjson_flag_double;
    v->doubleval = d;
}
...
const char* cjson_type_string(cjson_value* v)
{
    if (v) {
        switch (v->type) {
            ...
            case cjson_boolean: return "cjson_boolean"; 
